...
app.use((req, res, next) => {
  console.log(`[INGRESS] ${new Date().toISOString()} | ${req.method} ${req.url}`);
...
app.get("/api/session/:sessionId/agent-files", (req, res) => {
  try {
    const agentDir = path.join(UPLOADS_DIR, req.params.sessionId, "agent_files");
...
app.get("/api/session/:sessionId/agent-files/:filename/content", (req, res) => {
  try {
    const filePath = path.join(UPLOADS_DIR, req.params.sessionId, "agent_files", req.params.filename);
...
// Download agent file
app.get("/api/session/:sessionId/agent-files/:filename/download", (req, res) => {
  try {
    const filePath = path.join(UPLOADS_DIR, req.params.sessionId, "agent_files", req.params.filename);
...
app.post("/api/chat/:sessionId", async (req, res) => {
  ...
  const s0 = ensureSession(sessionId); // Ensure session is loaded in memory
...
process.on("unhandledRejection", (reason, promise) => {
  console.error("🛡️ UNHANDLED_REJECTION at:", promise, "reason:", reason);
  // Log but do NOT crash — keep the server alive
});

process.on("uncaughtException", (err) => {
  console.error("🛡️ UNCAUGHT_EXCEPTION (process kept alive):", err.stack || err.message);
  // Log but do NOT crash — prevents native module sync throws from killing the server
});
